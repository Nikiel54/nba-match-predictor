...
export default function PredictionContents({ prediction, teamData }) {
    const [imagesLoaded, setImagesLoaded] = useState(false); // pre-load images and display once all loaded


    const homeTeamName = teamData
        .find((team) => 
            prediction['home_team_id'] === team.id
        ).name;
    const awayTeamName = teamData
        .find((team) => 
    ...
    useEffect(() => {
        ...
        const preloadImages = async () => {
            try {
                // make some promises which resolve on img load
                const loadImg = (url) => {
        ...
        <>  
            {imagesLoaded ? (
                <>
                    ...
                    PREDICTED WINNER
                    <svg className="trophy-svg" xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24" width='10px' height="10px">
